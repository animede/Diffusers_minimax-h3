...
- **既知の改善余地**: エンコード位相が t2i_batch (0.9s) より重い (212.5s) のは、
  参照の Qwen3-VL ビジョンエンコードが**場面ごとに**走るため (~65s/場面)。参照は
  全場面共通なのでビジョン埋め込み・参照VAE潜在は場面間で再利用できるはず
  (エンコードを1回にすれば ~120s/枚 → 場面あたり限界 ~62s まで下がる見込み)。
  prompt_embeds は「参照トークン+プロンプトトークン」が一体でパックされるため、
  安易なテンソル切り貼りではなく encode_prompt の分解が要る — 未着手
...
- **ref2i_batch の参照エンコード共有**: 参照が全場面共通なのに Qwen3-VL ビジョン
  エンコード (~65s/場面) と参照VAEエンコードが場面ごとに走る。共有できれば
  164.9s/枚 → 場面あたり限界 ~62s の見込み(「参照付き静止画」セクションの
  改善余地メモ参照。encode_prompt の分解が要るため安易にはできない)
