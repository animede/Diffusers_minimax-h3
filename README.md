...
+ 投影TE + turbo での実測(96GB機、**全て 768² に揃えて計測**)。

| モード | 単発 | バッチ合計 | **1件あたり** | 短縮 |
|---|---|---|---|---|
| t2i 768² | 7.65s | 3場面 23.6s | **7.9s** | **0.97倍(効果なし)** |
| ref2i 768² | 79.3s | 3場面 140.9s | **47.0s** | **1.69倍** |
| i2va 5秒 768² | 103.1s | 2場面 150.1s | **75.0s** | **1.37倍** |

- **t2i のバッチはもう効かない**(0.97倍 = わずかに遅い)。元々「モデルのロード固定費を
  バッチ1回に償却する」機能だったので、**常駐化で償却すべき固定費が消えた**ため
  (常駐化前は 157s → 67.5s の 2.3倍だった)。
- **参照系は依然効く**(ref2i 1.69倍、i2va 1.37倍)。こちらは参照ビジョンエンコード ~47s が
  場面間で共有されるため。**バッチのステップ時間は単発と完全に一致**しており
  (ref2i 2.598s vs 単発 2.599s、i2va 7.321s vs 7.323s)、バッチ経路自体のオーバーヘッドは
  ゼロ。差は純粋に「エンコードを何回払うか」だけ。
- **「共有すれば縮む」モデルは両方で成立する**(単発 − バッチ1件あたり を、共有で消える
  エンコード `47×(場面数-1)/場面数` と比較):

  | | 実測削減 | 共有モデルの予測 | 差 |
  |---|---|---|---|
  | ref2i(3場面) | 32.3s/枚 | 31.3s/枚 | **+1.0s** |
  | i2va(2場面) | 28.1s/本 | 23.5s/本 | **+4.6s** |

  → **参照エンコードをリクエスト跨ぎでキャッシュすれば、静止画でも動画でも計算どおりの
  短縮が得られる**見込み(未実装。日付節 2026-08-12 参照)。
