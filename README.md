...
48GB機(PRO 5000 48GB + RTX 4000 20GB)・`H3_LOWVRAM=1` 系・768²。
...
| モード | 定常 | デノイズ | ピーク | 時間を食っている残りの固定費 |
|---|---|---|---|---|
| **t2i** 768² | **7.40s** | 2.40s | 45.0GB | VAE の CPU↔GPU 往復 ~3.3s |
| **t2va** 5秒 768² | **28.13s** | 14.94s | 45.6GB | デコード 7.05s |
| **ref2i** 768²(参照付き静止画) | **79.3s** | 7.8s | 45.4GB | 参照ビジョンエンコード **~47s** |
| **i2va** 5秒 768²(画像参照→動画) | **103.1s** | 22.0s | 45.9GB | 同 ~47s + t2va transformer の再ロード ~13s |
...
バッチの位相並べ替えは **`H3_LOWVRAM=1` 専用**(`app.py` の `if H3_LOWVRAM:`)。上の最速構成は
plain なので**逐次フォールバックになる**ため、下表は `H3_LOWVRAM=1 H3_KEEP_TRANSFORMER=1`
+ 投影TE + turbo での実測(96GB機、ピーク 46.7GB)。

| モード | 単発 | バッチ合計 | **1件あたり** | 短縮 |
|---|---|---|---|---|
| t2i 768² | 7.65s | 3場面 16.47s | **5.49s** | 1.39倍 |
| ref2i 768² | 79.3s | 3場面 148.9s | **49.6s** | 1.60倍 |
| i2va 5秒 768² | 103.1s | 2場面 201.4s | **100.7s** | 1.02倍 |

- **バッチの意義は常駐化で縮んだ**。元々「モデルのロード固定費をバッチ1回に償却する」
  ための機能だったので、固定費が消えた今 t2i は **1.39倍**まで低下した
  (常駐化前は 157s → 67.5s の **2.3倍**)。
- **ref2i は依然 1.6倍効く**。参照ビジョンエンコード ~47s が場面間で共有されるため
  (3場面で約90秒短縮 = 追加2場面ぶんの encode がまるごと消えている)。
- **ref2va(動画)のバッチはほぼ効かない**(1.02倍)。ただし**共有が効いていないのではない**:
  ログのタイムスタンプを見ると、**ref2i(3場面)も ref2va(2場面)も ~47-48秒のエンコード
  ブロックはちょうど1回だけ**で、場面数によらず共有されている。差が出るのは別の位相で、
  **バッチのデノイズ区間が単発より場面あたり重い**(動画: 2場面で84s = 42s/場面 に対し
  単発は 22s/場面。静止画: 3場面で32s = 10.7s/場面 に対し単発 7.8s/場面)。
  静止画では共有の利得が上回り、動画では相殺されて消える。**この「場面あたりが重くなる」
  原因は未特定**(要調査)。

**「共有すれば縮む」モデルの検算**(単発 − バッチ1件あたり を、共有で消えるエンコード
`47×(場面数-1)/場面数` と比較):

| | 実測削減 | 共有モデルの予測 | 差 |
|---|---|---|---|
| ref2i(3場面) | 29.7s/枚 | 31.3s/枚 | **-1.7s(ほぼ一致)** |
| i2va(2場面) | 2.4s/本 | 23.5s/本 | **-21.1s(大きく外れる)** |

→ **静止画ではモデルが成立し、動画では成立しない**。参照エンコードのキャッシュ化で
得られる利得は、静止画では見込みどおりだが、**動画では上記の未解明コストに食われる**
可能性が高い(下の「見込みの訂正」参照)。
- **参照バッチは `H3_TE_DEVICE` と併用不可**: 「TE用GPUに 24GB 以上が必要」というガードで
  400 拒否される。これは **32B TE の vision 活性化を前提にした閾値**で、3.11GB の
  投影TEには過大(16GB の 4060 Ti が弾かれる)。投影TE を同居させれば動く。**要見直し**。

### 個別の施策: 何がどれだけ効いて、出力は変わるのか

等価性 ◎ = 同一seedで**出力 MD5 一致**(数学的に無影響)/ ○ = ε級ドリフト(ビット一致
しないが目視同等)/ △ = 近似・別モデル(品質が変わりうる)。

| 施策 | 指定 | 実測効果 | 等価性 | 測定環境 |
|---|---|---|---|---|
| 量子化済みTEのディスクキャッシュ | `H3_TE_PREQUANT`(既定ON) | TEロード 53.0→**29.5s**、リクエスト合計 128.6→**83.2s**(-35%) | ◎ | 48GB機 |
| TE の未使用上位層の削除 | `H3_TE_PRUNE=1` | TEロード 42.3→**35.0s**(-17%) | ◎(mp4 バイト一致) | 48GB機 |
| TE を2枚目GPUへ常駐 | `H3_TE_DEVICE=cuda:1` | t2i 定常 78.4→**約35s**(-55%) | ○(sm 差・相対RMS 0.084%) | 48GB+20GB |
| transformer 常駐 | `H3_KEEP_TRANSFORMER=1` | 毎リクエストの再ロード固定費が消滅 → t2i **9.7s** / t2va **44.2s** | ◎ | 48GB+20GB |
| FirstBlockCache | `H3_CACHE=fbc`(既定) | デノイズ 157→**118s**(-25%、30step 中7スキップ) | △(ステップスキップ) | 96GB機 |
| lightx2v turbo LoRA | `H3_TURBO_LORA=1` | t2va 351.4→**143s**(2.6倍。デノイズ単体は 7.6倍) | △(4step 蒸留) | 48GB機 |
| 静止画の連番バッチ | `/api/t2i_batch` | 逐次 157→**67.5s/枚**(3場面。限界 ~31s/枚) | ◎ | 48GB機 |
| 参照KVプレフィックスの場面間共有 | `H3_REF_PREFIX_CACHE`(既定ON) | 参照エンコード 212.5→**83.1s**、ref2i バッチ 164.9→**116.7s/枚**(-29%) | ○(PSNR 21.9〜27.4dB) | 48GB機 |
| 参照付き静止画の超短尺化 | `still=1`(124→22フレーム) | デノイズ 102.2→**13.5s**(1/7.6) | —(尺が違うので比較対象外) | 48GB機 |
| 投影TE(32B TE の代替) | `H3_TE_PROJ` | t2i 65.7→**33.5s**、t2va 162.1→**143.5s** | △(PSNR 22.4dB・目視同等) | 96GB機 |

### VRAM を削った施策

| 施策 | 効果 |
|---|---|
| transformer の int8 量子化(`H3_TRANSFORMER_QUANT=int8`) | 66.3 → **34.0GB** |
| 投影TE の NF4 化(`H3_TE_PROJ`、既定 bnb-4bit) | TE 常駐 21.02 → **3.11GB** |
| TE の未使用上位層の削除(`H3_TE_PRUNE=1`) | 21.01 → **17.44GB** |
| video VAE の fp16 デコード(`H3_VIDEO_VAE_FP16=1`) | デコード位相 16.29 → **約11.4GB** |
| uint8 変換の逐次化(2026-08-10) | デコード位相の中間テンソル +2.65 → **+0.03GB** |
| デコード逆正規化の CPU 化(2026-08-12) | 768²×124フレームの一括 fp32 化 **838MiB** が GPU から消滅 |

これらを積んだ結果が冒頭の「[VRAM×機能マトリクス](#vram機能マトリクス2026-08-11-実測)」
(実 16GB カード単体・8GB×2 での動作)。
...
当初この2つを潰せば「**i2va 103s → 45s 程度**」と見積もったが、**その後のバッチ実測が
この見積もりを否定した**ので訂正しておく。バッチ経路は「参照エンコードを共有したら
どうなるか」を実際にやっている実験そのものであり、そこでの結果は:

- **静止画は見込みどおり**: ref2i の実測削減 29.7s/枚 に対し、共有モデルの予測 31.3s/枚
  (差 -1.7s)。**キャッシュ化の利得はほぼ計算どおりに出る**。
- **動画は出ない**: i2va の実測削減はわずか 2.4s/本、予測 23.5s/本 に対して -21.1s 外れる。
  エンコード自体は1回しか走っていない(ログで確認済み)のに、**バッチ側のデノイズが
  場面あたり 22s → 42s へ重くなり、共有の利得を食い潰している**。

したがって「**リクエスト跨ぎキャッシュ + 再ロード撤廃 → i2va 45s**」は**現時点では
根拠がない**。静止画(ref2i)については同種の改善が見込めるが、**動画は先に
「バッチのデノイズが場面あたり重くなる原因」を解明しない限り、短縮を約束できない**。
