...
- **結論**: この箱での静止画の連続生成は **`H3_LOWVRAM=1` のまま回すのが最速
  (~157s/枚)**。さらに縮めるにはバッチエンドポイント(下記「未着手の改善候補」)が本命
  — TE常駐のまま全プロンプトを先にエンコード → transformer を1回ロードして全場面を
  デノイズ → まとめてデコード、で固定費をバッチ全体で1回にすれば
  1枚あたり実質 ~35-40s (エンコード+デノイズ29s+デコード2s) まで下がる見込み
...
- **静止画のバッチエンドポイント (`/api/t2i_batch`)**: プロンプト配列を受けて
  「TE常駐のまま全プロンプトをエンコード → transformer 1回ロードで全場面をデノイズ →
  まとめてデコード」に並べ替え、`H3_LOWVRAM=1` の固定費 (~120s) をバッチ全体で1回に
  する。1枚あたり ~157s → ~35-40s の見込み(上の「連続生成の実測」参照。group モードは
  実測で不利と判明済みなので、この改善は lowvram=1 の並べ替えで作るのが正解)
