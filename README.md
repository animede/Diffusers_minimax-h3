...
### ただし出力はまだ壊れている(クラッシュは直ったが、絵が破綻する)

**この修正で「実行できる」ようにはなったが、出力品質は直っていない。** ユーザー指摘で発覚:

| 設定 | 症状 |
|---|---|
| turbo 4steps(パス2=**1**step) | **動く腕などの輪郭が二重になる**。手も崩れる |
| 30steps(パス2=**10**step) | 空・電線あたりに**格子状のモザイク** |

**ステップ数を増やしても消えない**(症状が変わるだけ)ので「パス2のステップ不足」ではない。
768²・upscale なしの同一 seed 対照では腕はクリーンなので、**アップスケール経路起因**で確定。
PR #14355 でレイアウト/パッチ規約が変わったことに 2x 空間補間が追随できていない疑いが濃く、
`denoiser_input_fields` の取り残し以外にも移行漏れが残っていると見られる(**要調査**)。

**当面の扱い**: `upscale=1` は**検証用途限定**。使うたびに `logger.warning` で既知の破綻を
知らせるようにした(黙って壊れた絵を返さないため)。

> **記録**: 修正直後、私(実装者)は 1536² の中央フレームを見て「破綻なし」と報告したが、
> **誤りだった** — 静止した中央付近だけを見ており、**動きのある腕**を見ていなかった。
> 動画の破綻は「動いている部位」に出るので、**静止フレーム1枚での判定は不十分**。
> 連続フレームを並べて動く部位を見ること。
