...
17.45GB の代替。制約: `<d>` タグ不可=音声参照で代替・細部は近似・ref2va の vision 経路
未検証)、②デコード位相が **7.53GB**(video VAE fp16 + uint8修正後。fp32 でも 14.2GB)、
...
| **16GB(~15.2)** | **不可**(TE 17.45 が載らない) | **`group` で成立見込み**: TE 3.11+ブロック1.4+活性化6.6=**11.1GB**、デコード 7.53GB。従来「不可」だった16GBに道が開く。**未実測** |
...
必要。投影TE の vision 経路は**未検証**(投影行列がテキストのみで校正されているため)。
...
| **投影4B NF4** | **3.11GB(実測)+ε** | **6〜8GB級で足りる見込み**(導出・未実測。ref2va の vision 経路も未検証) |
